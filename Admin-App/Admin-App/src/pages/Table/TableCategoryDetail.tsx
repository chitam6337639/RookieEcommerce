...
const TableCategoryDetail = () => {
    ...
    useEffect(() => {
        const fetchCategories = async () => {
            try {
                debugger
                if (!categoryId) return; // Đảm bảo categoryId không null hoặc undefined

                const subCategoryData = await getSubcategories(parseInt(categoryId)); // Chuyển đổi categoryId thành number 
    ...
    return (
        <DefaultLayout>
            <Breadcrumb pageName="CategoryDetail" />
            <div className="rounded-sm border border-stroke bg-white px-5 pt-6 pb-2.5 shadow-default dark:border-strokedark dark:bg-boxdark sm:px-7.5 xl:pb-1">
                <div className="flex justify-end mb-4">
                    <Link
                        to={`/forms/form-category?parentId=${subCategory.length > 0 ? subCategory[0].parentId : ""}`}
                                        ...
                                        {category.subCategories.length > 0 ? (
                                            <ul className="text-black dark:text-white">
                                                {category.subCategories.map((subCategory) => (
                                                    <li key={subCategory.categoryId} className="flex items-center justify-between">
